    ...
    public function tree($thread, &$out = null)
    {
        if (!empty($thread) && is_array($thread)) {
            ...
            foreach ($thread as $item) {
                $out .= "<li data-name='{$item->ingredient->slug}'><span><input type='number' maxlength='3' size='3' min='0' max='{$item->quantity}' data-id='{$item->id}'>/" . $item->quantity . ' ' . $item->ingredient->name;
